...
public class CameraFadeController : MonoBehaviour
{
    ...
    public void StartFade(CameraFadeAttributes cameraFadeAttributes)
    {
        ...
        currentFadeCoroutine = BasicImageFade(cameraFadeAttributes);
    ...
    private IEnumerator BasicImageFade(CameraFadeAttributes cameraFadeAttributes)
    {
        ...
        switch (cameraFadeAttributes.cameraFadeType)
        {
            case CAMERA_FADE_TYPE.IN:
                fadeDuration = cameraFadeAttributes.fadeInTime;
                currentColor = baseImageColor;
                targetFadeColor = Color.clear;
                break;

            case CAMERA_FADE_TYPE.OUT:
                fadeDuration = cameraFadeAttributes.fadeOutTime;
        ...
        if (cameraFadeAttributes.fadeTransitionWaitTime > 0)
        {
            yield return new WaitForSeconds(cameraFadeAttributes.fadeTransitionWaitTime);
        }
